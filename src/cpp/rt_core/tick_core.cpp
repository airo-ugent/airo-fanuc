...
void TickCore::install_trajectory(const Target& t) {
  ...
  //                tick whenever qd0 != 0.
  //   residue    — the splice's last knot is not (q0, qd0) but (q0, qd0) advanced by
  //                CapturePath::residue_ns, because the profile's duration is not a
  //                whole number of ticks (see capture.hpp). That knot IS the trajectory
  //                at its own time `residue`, so the next sample belongs at
  //                residue + itp. Without the term the handoff tick covers only
  //                (itp − residue) of a tick's travel — a commanded-velocity notch of
